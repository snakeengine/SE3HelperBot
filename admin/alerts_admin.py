...
# نظام البث/التخزين/الإحصاءات
from utils.alerts_broadcast import (
    ACTIVE_FILE,
    _load_json,
    _save_json,
    STATS_FILE,
...
def _load_draft() -> dict:
    ...
    d.setdefault("await", "")
    d.setdefault("ttl", 0)
    # مفاتيح ذكية اختيارية
    d.setdefault("ping_ttl", 0)            # مدة ظهور الـ ping (ثوانٍ)
    d.setdefault("active_for", 7 * 24 * 3600)  # بقاء التنبيه في الصندوق
...
def _menu_kb(lang: str) -> InlineKeyboardMarkup:
    ...
    kb.button(text=t(lang, "alerts.menu.jobs") or "🗓️ المهام المجدولة", callback_data="al:jobs")
    ...
    kb.button(text=t(lang, "alerts.menu.active") or "📥 الإشعارات النشطة", callback_data="al:active")
...
async def al_active(cb: CallbackQuery):
    ...
    if not items:
        ...
        await _safe_edit_text(
            cb,
            t(lang, "alerts.active.empty")
            or "قائمة الإشعارات — 0\nاختر معاينة أو حذف لإشعار محدد.",
            kb,
        )
...
@router.message(AlStates.wait_en, F.from_user.func(lambda u: _is_admin(u.id)))
async def capture_text_en(msg: Message, state: FSMContext):
    ...
    await msg.reply(f"تم الحفظ [EN] — أرسل العربية الآن\n[token:{tok2}]", reply_markup=ForceReply(selective=True))


@router.message(AlStates.wait_ar, F.from_user.func(lambda u: _is_admin(u.id)))
...
async def al_prev(cb: CallbackQuery):
    ...
    txt = (
        (t(lang, "alerts.preview.header") or "معاينة 👀")
        + f"\n\n[EN]\n{d.get('en') or '-'}\n\n[AR]\n{d.get('ar') or '-'}"
        + f"\n\n(kind={d.get('kind')}, lang_mode={d.get('lang_mode')})"
    )
    await _safe_edit_text(cb, txt)
    await cb.answer()
...
async def al_send(cb: CallbackQuery, state: FSMContext):
    ...
    _save_draft(d)
    await _safe_edit_text(
        cb, t(lang, "alerts.ask_ttl") or "أدخل مدة بقاء الرسالة بالثواني (0 يعني لا حذف)، مثال: 60"
    )
    await cb.answer()


@router.message(AlStates.wait_ttl, F.text.regexp(r"^\d{1,5}$") & F.from_user.func(lambda u: _is_admin(u.id)))
async def handle_ttl_send_now(msg: Message, state: FSMContext):
    ...
    ping_ttl = int(d.get("ping_ttl") or ttl)
    active_for = int(d.get("active_for") or (7 * 24 * 3600))
    ...
    _save_draft(d)
    await state.clear()
    await msg.reply((t(lang, "alerts.sent") or "تم الإرسال ✅") + f"\nsent={sent}, skipped={skipped}, failed={failed}")
...
async def al_sch(cb: CallbackQuery, state: FSMContext):
    ...
    lang = _L(cb.from_user.id)
    await _safe_edit_text(
        cb, (t(lang, "alerts.ask_when") or "أدخل وقت الجدولة (YYYY-MM-DD HH:MM)") + "\nمثال: 2025-08-26 21:30"
    )
    await cb.answer()


@router.message(F.text.regexp(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$") & F.from_user.func(lambda u: _is_admin(u.id)))
...
async def al_cfg(cb: CallbackQuery, state: FSMContext):
    ...
    kb.button(
        text=("🔕 Quiet: OFF" if not cfg.get("quiet_enabled") else "🔔 Quiet: ON"),
...
async def al_cfg_toggle(cb: CallbackQuery, state: FSMContext):
    if not _is_admin(cb.from_user.id):
        return await cb.answer("no", show_alert=True)
    cfg = get_config()
    set_config({"enabled": not bool(cfg.get("enabled"))})
    await cb.answer("OK")
...
async def al_cfg_quiet(cb: CallbackQuery, state: FSMContext):
    ...
    lang = _L(cb.from_user.id)
    txt = (
        t(lang, "alerts.settings.ask_quiet_hours")
        or "أدخل ساعات الهدوء hh:mm-hh:mm (مثال 22:00-08:00)\nاكتب off لإيقاف الهدوء كليًا."
    )
    await _safe_edit_text(cb, txt)
    await cb.answer()


@router.message(AlStates.wait_quiet, F.from_user.func(lambda u: _is_admin(u.id)))
...
@router.message(AlStates.wait_rate, F.text.regexp(r"^\d{1,4}$") & F.from_user.func(lambda u: _is_admin(u.id)))
...
@router.message(AlStates.wait_maxw, F.text.regexp(r"^\d{1,3}$") & F.from_user.func(lambda u: _is_admin(u.id)))
...
async def al_cfg_actd(cb: CallbackQuery, state: FSMContext):
    ...
    lang = _L(cb.from_user.id)
    await _safe_edit_text(
        cb, t(lang, "alerts.settings.ask_active_days") or "أرسل عدد الأيام النشطة (استهداف المستخدمين خلال X يوم):"
    )
    await cb.answer()


@router.message(AlStates.wait_actd, F.text.regexp(r"^\d{1,4}$") & F.from_user.func(lambda u: _is_admin(u.id)))
    ...
    _save_draft(
        {
            "en": "",
            "ar": "",
            "lang_mode": DEFAULT_LANG_MODE,
            "kind": DEFAULT_KIND,
            "await": "",
            "ttl": 0,
            "ping_ttl": 0,
            "active_for": 7 * 24 * 3600,
            "dedupe_key": "",
        }
    )
    await state.clear()
