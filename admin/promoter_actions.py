...
def _save(d): STORE_FILE.write_text(json.dumps(d, ensure_ascii=False, indent=2), encoding="utf-8")
...
# ====== أكشن عام ======
async def _finish(cb: CallbackQuery, note: str):
    try:
...
async def action_basic(cb: CallbackQuery):
    ...
    await _notify_user(cb, int(uid), _msg(lang, user_key, fb))
    await _finish(cb, "✅")
...
async def action_ban(cb: CallbackQuery):
    ...
    _save(d)
    await _notify_user(cb, int(uid), _msg(lang, "prom.user.banned", f"🚫 تم حظرك لمدة {days} يومًا."))
    await _finish(cb, "✅")
...
async def action_unban(cb: CallbackQuery):
    ...
    u["banned_until"] = 0
    _save(d)
    await _notify_user(cb, int(uid), _msg(lang, "prom.user.unbanned", "♻️ تم إزالة الحظر عن حسابك."))
    await _finish(cb, "✅")
...
async def action_delete(cb: CallbackQuery):
    ...
    _save(d)
    await _notify_user(cb, int(uid), _msg(lang, "prom.user.deleted", "🗑 تم حذف طلبك."))
    await _finish(cb, "✅")
