...
# Ù„Ø§ Ù†Ø¹ØªØ±Ø¶ back_to_menu ØØªÙ‰ ÙŠØ¨Ù‚Ù‰ Ø¹Ù†Ø¯ persistent_menu
...
# ===== Ù†Øµ Ø«Ù†Ø§Ø¦ÙŠ Ø§Ù„Ù„ØºØ© Ù…ØÙ„ÙŠ =====
...
async def help_handler_target(user_id: int, send_func):
    ...
    await send_func(
        L(
            ...
            "Ø§Ø®ØªØ± ÙØ¦Ø© Ø§Ù„Ù…Ø´ÙƒÙ„Ø© Ù„Ù„ØØµÙˆÙ„ Ø¹Ù„Ù‰ Ø®Ø·ÙˆØ§Øª Ù…ÙØµÙ„Ø©. Ø¥Ù† Ù„Ù… ØªÙØÙ„ Ù…Ø´ÙƒÙ„ØªÙƒØŒ Ø§ÙØªØ <code>/report</code> Ø£Ùˆ Ø§Ø³ØªØ®Ø¯Ù… Ø§Ù„Ø¯Ø±Ø¯Ø´Ø© Ø§Ù„ØÙŠÙ‘Ø©.",
...
# ===== Ø±Ø¬ÙˆØ¹ Ù„ØµÙØØ© Ø§Ù„Ù€FAQ Ø§Ù„Ø±Ø¦ÙŠØ³ÙŠØ© =====
...
async def app_not_open(callback: CallbackQuery):
    ...
    text = L(
        lang,
        "ðŸ”§ <b>ØÙ„ Ù…Ø´ÙƒÙ„Ø©: Ø§Ù„ØªØ·Ø¨ÙŠÙ‚ Ù„Ø§ ÙŠÙØªØ</b>\n"
        "1) ØØ¯Ù‘Ø« Ù„Ø£ØØ¯Ø« Ø¥ØµØ¯Ø§Ø± Ù…Ù† Ø¯Ø§Ø®Ù„ Ø§Ù„Ø¨ÙˆØª (Ù‚Ø³Ù… Ø§Ù„ØªØ·Ø¨ÙŠÙ‚).\n"
        "2) Ø¹Ø·Ù‘Ù„ Ù…Ø¤Ù‚ØªÙ‹Ø§ VPN/ØØ§Ø¬Ø¨ Ø§Ù„Ø¥Ø¹Ù„Ø§Ù†Ø§Øª/DNS Ø§Ù„Ù…Ø®ØµØµ.\n"
        ...
        "4) ØªØ£ÙƒØ¯ Ù…Ù† ÙˆØ¬ÙˆØ¯ >500MB Ù…Ø³Ø§ØØ© Ø«Ù… Ø£Ø¹Ø¯ ØªØ´ØºÙŠÙ„ Ø§Ù„Ø¬Ù‡Ø§Ø².\n"
        "5) Ø¥Ù† Ø§Ø³ØªÙ…Ø±Øª Ø§Ù„Ù…Ø´ÙƒÙ„Ø©: Ø§ØØ°Ù Ø§Ù„ØªØ·Ø¨ÙŠÙ‚ ÙˆØ«Ø¨ØªÙ‡ Ù…Ù† Ø¬Ø¯ÙŠØ¯.\n"
...
async def app_slow(callback: CallbackQuery):
    ...
    text = L(
        ...
        "â€¢ ØØ¯Ù‘Ø« Ø§Ù„ØªØ·Ø¨ÙŠÙ‚ ÙˆØ§Ù…Ø³Ø Ø§Ù„ÙƒØ§Ø´.\n"
        "â€¢ Ø¹Ø·Ù‘Ù„ Ø£ÙˆØ¶Ø§Ø¹ ØªÙˆÙÙŠØ± Ø§Ù„Ø·Ø§Ù‚Ø© Ø§Ù„Ø´Ø¯ÙŠØ¯Ø©.\n"
        "â€¢ Ø¥Ù† Ù„Ù… ÙŠØªØØ³Ù†: Ø£Ø¹Ø¯ Ø§Ù„ØªØ«Ø¨ÙŠØª.\n"
...
async def menu_not_showing(callback: CallbackQuery):
    ...
    text = L(
        ...
        "â€¢ ØØ¯Ù‘Ø« Ø§Ù„ØªØ·Ø¨ÙŠÙ‚.\n"
        "â€¢ Ø§Ù…Ø³Ø Ø§Ù„ÙƒØ§Ø´ ÙˆØ§Ù„Ø¨ÙŠØ§Ù†Ø§Øª Ø«Ù… Ø§ÙØªØÙ‡ Ù…Ø¬Ø¯Ø¯Ù‹Ø§.\n"
...
async def game_not_working(callback: CallbackQuery):
    ...
    text = L(
        lang,
        "ðŸŽ® <b>ØÙ„ Ù…Ø´ÙƒÙ„Ø©: Ø§Ù„Ù„Ø¹Ø¨Ø© Ù„Ø§ ØªØ¹Ù…Ù„</b>\n"
        "1) ØØ¯Ù‘Ø« Ø§Ù„Ù„Ø¹Ø¨Ø© ÙˆØ§Ù„ØªØ·Ø¨ÙŠÙ‚.\n"
        ...
        "5) Ø£Ø«Ù†Ø§Ø¡ ØµÙŠØ§Ù†Ø© Ø§Ù„Ø®Ø§Ø¯Ù… Ù‚Ø¯ ØªØªØ¹Ø·Ù„ Ù…Ø¤Ù‚ØªÙ‹Ø§ â€” Ø¬Ø±Ù‘Ø¨ Ù„Ø§ØÙ‚Ù‹Ø§.\n"
...
async def game_crash(callback: CallbackQuery):
    ...
    text = L(
        ...
        "â€¢ Ø£ÙØ±Øº Ù…Ø³Ø§ØØ© ÙˆØ±Ø§Ù… ÙƒØ§ÙÙŠØ©.\n"
        "â€¢ ØØ¯Ù‘Ø« Ø§Ù„Ù†Ø¸Ø§Ù…/ØªØ¹Ø±ÙŠÙØ§Øª Ø§Ù„Ø±Ø³ÙˆÙ… Ø¥Ù† ÙˆÙØ¬Ø¯Øª.\n"
...
async def help_reseller(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=L(lang, "Ø§Ù„ØªØÙ‚Ù‚/Ø¨Ù„Ø§Øº Ø¹Ù† Ù…Ø²ÙŠÙ", "Verify/Report fake"),  callback_data="help_reseller_fake")],
        [InlineKeyboardButton(text=L(lang, "â¬…ï¸ Ø±Ø¬ÙˆØ¹", "â¬…ï¸ Back"),                        callback_data="back_to_help")],
    ])
    await callback.message.edit_text(
...
async def reseller_not_responding(callback: CallbackQuery):
    ...
    text = L(
        ...
        "â€¢ Ø§Ù„Ù…Ù‡Ù„Ø© Ø§Ù„Ù…Ø¹ØªØ§Ø¯Ø© Ù„Ù„Ø±Ø¯ ØØªÙ‰ 24 Ø³Ø§Ø¹Ø©.\n"
        "â€¢ ØªÙˆØ§ØµÙ„ Ø¹Ø¨Ø± Ø§Ù„Ù‚Ù†Ø§Ø© Ø§Ù„Ø¸Ø§Ù‡Ø±Ø© Ø¯Ø§Ø®Ù„ Ø§Ù„Ø¨ÙˆØª ÙÙ‚Ø·.\n"
        "â€¢ Ø¥Ù† ØªØ¬Ø§ÙˆØ² 24 Ø³Ø§Ø¹Ø© Ø¯ÙˆÙ† ØªØØ¯ÙŠØ«: Ø§ÙØªØ <code>/report</code> Ù…Ø¹ Ø±Ù‚Ù… Ø§Ù„Ø·Ù„Ø¨ ÙˆÙˆØ³ÙŠÙ„Ø© Ø§Ù„Ø¯ÙØ¹.\n"
...
async def reseller_fake(callback: CallbackQuery):
    ...
    text = L(
        lang,
        "ðŸ›¡ï¸ <b>Ø§Ù„ØªØÙ‚Ù‚ ÙˆØ§Ù„Ø¥Ø¨Ù„Ø§Øº</b>\n"
        "â€¢ Ù„Ø§ ØªØ¯ÙØ¹ Ø®Ø§Ø±Ø¬ Ø§Ù„Ù‚Ù†ÙˆØ§Øª Ø§Ù„Ø±Ø³Ù…ÙŠØ© Ø¯Ø§Ø®Ù„ Ø§Ù„Ø¨ÙˆØª.\n"
        "â€¢ Ø§Ø·Ù„Ø¨ Ø¥Ø«Ø¨Ø§Øª Ø§Ù„Ù‡ÙˆÙŠØ© Ø¯Ø§Ø®Ù„ Ø§Ù„Ø¨ÙˆØª (ØØ³Ø§Ø¨ Ù…ÙˆØ«Ù‘Ù‚/Ù…Ø¹Ø±Ù‘Ù).\n"
        "â€¢ Ø£Ø¨Ù„Øº ÙÙˆØ±Ù‹Ø§ Ø¹Ø¨Ø± <code>/report</code> ÙˆØ£Ø±ÙÙ‚ Ø§Ù„Ù…ØØ§Ø¯Ø«Ø§Øª ÙˆØ§Ù„ÙÙˆØ§ØªÙŠØ±.\n"
...
async def error_code(callback: CallbackQuery):
    ...
    text = L(
        ...
        "â€¢ 401/403: ØµÙ„Ø§ØÙŠØ§Øª ØºÙŠØ± ÙƒØ§ÙÙŠØ© â€” Ø³Ø¬Ù‘Ù„ Ø§Ù„Ø¯Ø®ÙˆÙ„ Ø£Ùˆ ÙŠÙ„Ø²Ù… Ø§Ø´ØªØ±Ø§Ùƒ.\n"
        "â€¢ 404: Ø§Ù„Ø¹Ù†ØµØ± ØºÙŠØ± Ù…ØªØ§Ø Ø£Ùˆ ØªÙ… ØØ°ÙÙ‡.\n"
        "â€¢ 406/415: Ø¥ØµØ¯Ø§Ø± Ù‚Ø¯ÙŠÙ… Ø£Ùˆ ØµÙŠØºØ© ØºÙŠØ± Ù…Ø¯Ø¹ÙˆÙ…Ø© â€” ØØ¯Ù‘Ø« Ø§Ù„ØªØ·Ø¨ÙŠÙ‚.\n"
        "â€¢ 429: Ù…ØØ§ÙˆÙ„Ø§Øª ÙƒØ«ÙŠØ±Ø© â€” Ø§Ù†ØªØ¸Ø± Ø¯Ù‚Ø§Ø¦Ù‚ Ø«Ù… ØØ§ÙˆÙ„.\n"
        "â€¢ 500/502/503: Ù…Ø´ÙƒÙ„Ø© Ø®Ø§Ø¯Ù… â€” Ø¬Ø±Ù‘Ø¨ Ù„Ø§ØÙ‚Ù‹Ø§.\n"
...
async def error_unexpected(callback: CallbackQuery):
    ...
    text = L(
        ...
        "2) Ø§Ù…Ø³Ø ÙƒØ§Ø´ Ø§Ù„ØªØ·Ø¨ÙŠÙ‚ ÙˆØØ¯Ù‘Ø«Ù‡.\n"
