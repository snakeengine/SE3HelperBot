...
override_var "CRYPTOPAY_TOKEN"
override_var "CRYPTO_PAY_TOKEN"

set -a; . ./.env 2>/dev/null || true; set +a
echo "[BOOT] MAINTENANCE=${MAINTENANCE:-0} | WIPE_DATA=${WIPE_DATA:-0} | BOT? $([ -n "${BOT_TOKEN:-}" ] && echo yes || echo no) | CRYPTOPAY? $([ -n "${CRYPTOPAY_TOKEN:-${CRYPTO_PAY_TOKEN:-}}" ] && echo yes || echo no)"
